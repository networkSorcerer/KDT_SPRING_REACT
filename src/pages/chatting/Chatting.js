...
const Chatting = () => {
  ...
  const { roomId } = useParams(); // 채팅 방 번호,  새로운 방 개설 , 기존의 방에 대한 진입
  const [sender, setSender] = useState(""); // 보낸 사람
  const [roomName, setRoomName] = useState(""); // 채팅 방이름
  const ws = useRef(null); // 웹 소켓 객체 생성, 소켓 연결 정보를 유지 해야 하지만 렌더링과는 무관
  ...
  const onClickMsgSend = (e) => {
    ...
    setInputMsg(""); // 전송 이후 입력 창 지우기
  };

  const onEnterKey = (e) => {
    // 엔터키 입력시 , 공백 제거 후 비어 있지 않으면
  ...
  }, [sender, socketConnected]); // sender와 socketConnected에 의존
